...
- 目标新增响应没有订阅 ID 时，删除会直接使用官方 TMDB 媒体键接口，不再请求可能返回 404 的目标订阅列表。
- 豆瓣来源订阅会复用 MoviePilot 内部媒体识别链，使用标题、年份、类型和季将豆瓣 ID 对应到 TMDB ID 后再转发；转换失败时保留为“未识别”。
- 在 MoviePilot 原生订阅页面取消订阅时，会通过 `SubscribeDeleted` 事件同步取消目标端订阅并清除插件历史。
- 筛选“异常”或“未识别”状态后会显示“一键清除”按钮，沿用现有逐条删除接口，无额外批量 API。
