...
插件主页面使用 Vue 联邦组件渲染。每条 API 数据严格只包含 `id`、`title`、`category`、`subscribe_time`、`release_year` 和 `status`。状态仅可能是“已暂停、双重订阅、未识别、异常”。页面顶部提供四种状态数量胶囊、点击筛选、“筛选”和“多选”按钮，底部提供页码、输入框和跳转按钮。
