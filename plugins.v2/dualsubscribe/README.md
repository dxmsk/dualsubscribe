...
插件主页面使用 Vue 联邦组件渲染。每条 API 数据包含 `id`、`title`、`category`、`subscribe_time`、`release_year`、`status` 和 `poster`。海报在固定显示框内保持原比例完整缩放，不裁剪、不拉伸。状态仅可能是“已暂停、双重订阅、未识别、异常”，其中已暂停为橙灰色、双重订阅为紫蓝色、未识别为黄色、异常为醒目红色。页面顶部提供四种状态数量胶囊、点击筛选、“筛选”和“多选”按钮，底部提供页码、输入框和跳转按钮。
